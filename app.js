...
function render(ms){
  ...
  ctx.save();ctx.translate(w/2,sceneY);ctx.scale(sceneScale,sceneScale);if(state?.spaces && window.WorldRooms?.current && state.spaces[WorldRooms.current]?.kind!=='workshop')SpaceArt.draw(ctx,state.spaces[WorldRooms.current].kind,time,state,state.spaces[WorldRooms.current].name);else drawStation(ctx);
...
async function act(action,value){if(busy)return;busy=true;update();try{const result=await api('/api/action',{action,bird:['gift','whistle','teach_words','visit','nursery','travel','tinker','share_part','interact','build_structure','donate_parts','player_interact','create_world','salvage_supplies'].includes(action)?selected:'pip',value});setState(result.state);if(['tinker','share_part','interact','build_structure','donate_parts','player_interact','create_world','salvage_supplies'].includes(action)){ $('bubble').hidden=true;toast(result.reply); }else speak(result.reply);if(action==='hatch')selected='pip';chirp(['tune','teach','whistle'].includes(action));return true}catch(e){toast(e.message);try{setState(await api('/api/state'))}catch{}return false}finally{busy=false;update()}}
