...
Runtime v0.3.1.2 extends v0.3.0 with:
    - explicit `morphogenesis NAME { ... }` declarations
    - explicit `morph NAME` event statements
    - bounded persistent Ψ-node growth
    - bounded persistent τ-style graph connections
    - bounded low-rank numeric operator growth
    - deterministic hash-derived morphogenesis draws
    - checkpoint/restart preservation of grown structure
    - morphology counters exposed to XC measures

The morphogenesis layer is deliberately constrained: it can create numeric state
nodes, graph edges, and numeric operator terms. It cannot generate host-language
code, shell commands, network access, or undeclared world capabilities.
...
RUNTIME_VERSION = "0.3.1.2"
IR_VERSION = "0.3.1"
MEMORY_FORMAT_VERSION = "0.2"
CHECKPOINT_FORMAT_VERSION = "0.2"
...
class Parser:
    ...
    def parse(self) -> dict[str, Any]:
        if self.cur.value != "xembra":
            raise XCParseError(f"Runtime v0.3.1.2 expects one top-level xembra block; got {self.cur.value!r}")
        p = self.parse_xembra()
        self.skip_semis()
        if self.cur.kind != "EOF":
            raise XCParseError(f"Unexpected top-level token {self.cur.value!r} at {self.cur.line}:{self.cur.col}")
        return {"ir_version":IR_VERSION, "runtime_min_version":"0.3.1.2", "program":p}
    ...
    def parse_xembra(self):
        ...
        p = {
            ...
            "observations":{}, "events":{}, "morphogenesis":{},
        }
        while not self.accept("}"):
            ...
            if kw == "seed":
            ...
            elif kw == "morphogenesis":
                n,d = self.parse_assignment_block("morphogenesis")
                p["morphogenesis"][n] = d
            elif kw == "matrix":
                ...
                tok = self.cur
                raise XCParseError(f"{kw} is not implemented by XC Runtime v0.3.1.2 (at {tok.line}:{tok.col})")
            else:
    ...
    def parse_event(self):
        self.expect("event"); event_name=self.expect_kind("IDENT").value; self.expect("{")
        stmts=[]
        while not self.accept("}"):
            self.skip_semis()
            if self.at("}"): self.i += 1; break
            ...
            if self.accept("morph"):
                morph_name=self.expect_kind("IDENT").value
                stmts.append({"kind":"morph","name":morph_name}); self.skip_semis(); continue
            target=self.expect_kind("IDENT").value; self.expect("<-"); e=self.parse_expression()
            stmts.append({"kind":"update","target":target,"expr":e}); self.skip_semis()
        self.skip_semis(); return event_name,{"statements":stmts}
...
class Runtime:
    def __init__(self, ir: dict[str,Any], memory_data: dict[str,Any] | None=None, checkpoint_data: dict[str,Any] | None=None):
        ...
        self.observations={}
        self.morph_specs={}; self.morph_state={}
    ...
    def _initialize(self):
        if len(self.p["states"])!=1: raise XCValidationError("XC Runtime v0.3.1.2 supports exactly one state block")
        ...
        # Structural morphogenesis configuration and persistent state.
        for n,a in self.p.get("morphogenesis",{}).items():
            spec={k:self.eval_expr(v,{}) for k,v in a.items()}
            probability=float(spec.get("probability",0.05))
            max_nodes=int(spec.get("max_nodes",48))
            max_connections=int(spec.get("max_connections",96))
            max_operators=int(spec.get("max_operators",12))
            operator_every=int(spec.get("operator_every",3))
            operator_gain=float(spec.get("operator_gain",0.03))
            node_noise=float(spec.get("node_noise",0.08))
            min_weight=float(spec.get("min_weight",0.50))
            max_weight=float(spec.get("max_weight",1.00))
            targets=spec.get("target_matrices",list(self.matrices.keys()))
            if not isinstance(targets,list) or not all(isinstance(x,str) for x in targets):
                raise XCValidationError(f"morphogenesis {n} target_matrices must be a list of strings")
            if not (0.0 <= probability <= 1.0):
                raise XCValidationError(f"morphogenesis {n} probability must be in [0,1]")
            if max_nodes < 1 or max_connections < 0 or max_operators < 0:
                raise XCValidationError(f"morphogenesis {n} capacities are invalid")
            if operator_every < 1:
                raise XCValidationError(f"morphogenesis {n} operator_every must be >=1")
            if not (0.0 <= operator_gain <= 0.25):
                raise XCValidationError(f"morphogenesis {n} operator_gain must be in [0,0.25]")
            if not (0.0 <= node_noise <= 1.0):
                raise XCValidationError(f"morphogenesis {n} node_noise must be in [0,1]")
            if not (0.0 <= min_weight <= max_weight <= 1.0):
                raise XCValidationError(f"morphogenesis {n} connection weights must satisfy 0<=min<=max<=1")
            for target in targets:
                if target not in self.matrices:
                    raise XCValidationError(f"morphogenesis {n} targets undeclared matrix {target}")
                if shape(self.matrices[target]) != (len(self.field_names),len(self.field_names)):
                    raise XCValidationError(
                        f"morphogenesis {n} target matrix {target} must match state dimension"
                    )
            self.morph_specs[n]={
                ...
                "operator_every":operator_every,
                "operator_gain":operator_gain,
                "node_noise":node_noise,
                "min_weight":min_weight,
                "max_weight":max_weight,
                "target_matrices":list(targets),
            }
            self.morph_state[n]={
                "attempts":0,
                "events":0,
    ...
    def state_env(self,vector=None):
        ...
        env.update({name:self.effective_matrix(name) for name in self.matrices})
    ...
    def validate(self):
        ...
        for pname,pol in self.p["policies"].items():
            if pol.get("select")!="argmax": raise XCValidationError(f"policy {pname}: Runtime v0.3.1.2 supports only select argmax")
        ...
        for ename,e in self.p["events"].items():
            ...
            for s in e["statements"]:
                if s["kind"]=="observe":
                    ...
                    if s["name"] not in self.morph_specs:
                        raise XCValidationError(f"event {ename} morphs undeclared morphogenesis block {s['name']}")
                elif s["kind"]=="update":
    ...
    # ------------------------------------------------------------------
    # Bounded structural morphogenesis
    # ------------------------------------------------------------------
    ...
    def _unit(cls,v):
        n=cls._vnorm(v)
        if n<=1e-15: return [0.0 for _ in v]
    ...
    @staticmethod
    def _matrix_add(a,b):
        if shape(a)!=shape(b): raise XCRuntimeError("matrix add shape mismatch")
        return [[float(x)+float(y) for x,y in zip(ra,rb)] for ra,rb in zip(a,b)]

    @staticmethod
    def _matrix_scale(a,s):
        return [[float(s)*float(x) for x in row] for row in a]

    @staticmethod
    def _frobenius(a):
        return math.sqrt(sum(float(x)*float(x) for x in flatten(a)))

    def effective_matrix(self,name):
        if name not in self.matrices: raise XCRuntimeError(f"Unknown matrix {name}")
        out=[[float(x) for x in row] for row in self.matrices[name]]
        for block in self.morph_state.values():
            for op in block["operators"]:
                if op["target_matrix"] != name: continue
                out=self._matrix_add(out,self._matrix_scale(op["matrix"],op["gain"]))
        return out

    def _hash_u64(self,*parts):
        raw="|".join(str(x) for x in parts).encode("utf-8")
        d=hashlib.sha256(raw).digest()
        return int.from_bytes(d[:8],"big",signed=False)

    def _rand01(self,block_name,attempt,slot):
        seed=int(self.p.get("seed") or 0)
        source=self.ir.get("source",{}).get("sha256","")
        x=self._hash_u64(seed,source,self.event_counter,block_name,attempt,slot)
        return x / float((1<<64)-1)

    def _rand_range(self,block_name,attempt,slot,lo,hi):
        return float(lo) + (float(hi)-float(lo))*self._rand01(block_name,attempt,slot)

    def morphology_summary(self,name=None):
        names=[name] if name is not None else list(self.morph_state)
        out={}
        for n in names:
            if n not in self.morph_state: raise XCRuntimeError(f"Unknown morphogenesis block {n}")
            s=self.morph_state[n]
            out[n]={
                "attempts":int(s["attempts"]),
                "events":int(s["events"]),
                "last_triggered":bool(s["last_triggered"]),
                "last_kind":str(s["last_kind"]),
                "nodes":len(s["nodes"]),
    ...
    def morph(self,name):
        if name not in self.morph_specs: raise XCRuntimeError(f"Unknown morphogenesis block {name}")
        spec=self.morph_specs[name]; ms=self.morph_state[name]
        attempt=int(ms["attempts"])
        ms["attempts"]=attempt+1
        ms["last_triggered"]=False
        ms["last_kind"]="NONE"

        draw=self._rand01(name,attempt,"trigger")
        if draw >= spec["probability"]:
            return False

        d=len(self.field_names)
        if ms["nodes"]:
            ms["nodes"][0]["vector"]=list(self.state)

        made_node=False; made_connection=False; made_operator=False
        new_id=None

        if len(ms["nodes"]) < spec["max_nodes"]:
            salience=0.5
            if self.current_observation_name is not None:
                salience=float(self.observations[self.current_observation_name].get("salience",0.5))
            vec=[]
            for i in range(d):
                obs=float(self.current_observation[i]) if i < len(self.current_observation) else 0.0
                noise=self._rand_range(name,attempt,f"noise:{i}",-spec["node_noise"],spec["node_noise"])
                value=0.62*float(self.state[i]) + 0.28*obs + 0.10*float(self.memory_vector(next(iter(self.memory_specs))) [i] if self.memory_specs else 0.0) + noise
                vec.append(max(-1.0,min(1.0,value)))
            new_id=f"psi_{len(ms['nodes']):03d}"
            ms["nodes"].append({
                "id":new_id,
                "kind":"experience",
                "vector":vec,
                "created_event":int(self.event_counter),
                "salience":float(salience),
            })
            made_node=True

        if new_id is not None and len(ms["connections"]) < spec["max_connections"]:
            prior=ms["nodes"][:-1]
            if prior:
                # Strong core bias, but deterministic branching remains possible.
                if len(prior)==1 or self._rand01(name,attempt,"edge_source") < 0.70:
                    source="psi_core"
                else:
                    idx=int(self._rand01(name,attempt,"edge_index")*len(prior))
                    idx=min(len(prior)-1,max(0,idx))
                    source=prior[idx]["id"]
                weight=self._rand_range(
                    name,attempt,"edge_weight",spec["min_weight"],spec["max_weight"]
                )
                ms["connections"].append({
                    "source":source,
                    "target":new_id,
                    "weight":weight,
        ...
        # If node capacity is exhausted, bounded rewiring can still occur.
        elif len(ms["connections"]) < spec["max_connections"] and len(ms["nodes"]) > 1:
            ids=[n["id"] for n in ms["nodes"]]
            ia=int(self._rand01(name,attempt,"rewire_a")*len(ids)) % len(ids)
            ib=int(self._rand01(name,attempt,"rewire_b")*len(ids)) % len(ids)
            if ia==ib: ib=(ib+1)%len(ids)
            ms["connections"].append({
                "source":ids[ia],
                "target":ids[ib],
                "weight":self._rand_range(
                    name,attempt,"rewire_weight",spec["min_weight"],spec["max_weight"]
                ),
                "type":"rewire",
                "created_event":int(self.event_counter),
            })
        ...
        prospective_event=int(ms["events"])+1
        if (prospective_event % spec["operator_every"] == 0
                and len(ms["operators"]) < spec["max_operators"]
                and spec["target_matrices"]):
            target_index=int(
                self._rand01(name,attempt,"operator_target")*len(spec["target_matrices"])
            )
            target_index=min(len(spec["target_matrices"])-1,max(0,target_index))
            target=spec["target_matrices"][target_index]

            a=self._unit([
                float(self.state[i]) - 0.5*float(self.current_observation[i])
                for i in range(d)
            ])
            b=self._unit([
                float(self.current_observation[i]) + 0.25*float(self.state[i])
                for i in range(d)
            ])
            if self._vnorm(a)<=1e-15:
                a=self._unit([
                    self._rand_range(name,attempt,f"opa:{i}",-1.0,1.0) for i in range(d)
                ])
            if self._vnorm(b)<=1e-15:
                b=self._unit([
                    self._rand_range(name,attempt,f"opb:{i}",-1.0,1.0) for i in range(d)
                ])
            mat=self._outer(a,b)
            fn=self._frobenius(mat)
            if fn>1.0: mat=self._matrix_scale(mat,1.0/fn)
            gain=spec["operator_gain"] * self._rand_range(name,attempt,"operator_gain",0.45,1.0)
            ms["operators"].append({
                "id":f"op_{len(ms['operators'])+1:03d}",
        ...
        if not (made_node or made_connection or made_operator):
            return False

        ms["events"] += 1
        ms["last_triggered"]=True
        kinds=[]
        if made_node: kinds.append("PSI")
        if made_connection: kinds.append("TAU")
        if made_operator: kinds.append("OP")
        ms["last_kind"]="+".join(kinds)
        return True
    ...
    def eval_expr(self,node,env):
        ...
        if k=="call":
            ...
            if name in {"morph_node_count","morph_connection_count","morph_operator_count","morph_event_count"}:
                self._arity(name,args,1)
                if not isinstance(args[0],str): raise XCRuntimeError(f"{name} argument must be morphogenesis block name string")
                block=args[0]
                if block not in self.morph_state: raise XCRuntimeError(f"Unknown morphogenesis block {block}")
                s=self.morph_state[block]
                key={
                    "morph_node_count":"nodes",
                    "morph_connection_count":"connections",
                    "morph_operator_count":"operators",
                    "morph_event_count":"events",
                }[name]
                return float(len(s[key]) if key in {"nodes","connections","operators"} else s[key])
    ...
    def snapshot(self,step):
        return {
            ...
            "morphology":self.morphology_summary(),
        }
    ...
    def export_checkpoint(self):
        return {
            ...
            "event_counter":self.event_counter,
            "memory":self.export_memory(),
            "morphogenesis":jsonable(self.morph_state),
            "rng_state":None,
            "rng_note":"Morphogenesis uses deterministic SHA-256-derived draws from seed/source/event/attempt; no mutable PRNG state is required.",
        }
    ...
    def load_checkpoint_data(self,data):
        ...
        stored_morph=data.get("morphogenesis",{})
        if stored_morph:
            for name,stored in stored_morph.items():
                if name not in self.morph_specs:
                    raise XCValidationError(f"Checkpoint contains unknown morphogenesis block {name}")
                spec=self.morph_specs[name]
                nodes=list(stored.get("nodes",[]))
                connections=list(stored.get("connections",[]))
                operators=list(stored.get("operators",[]))
                if len(nodes)>spec["max_nodes"] or len(connections)>spec["max_connections"] or len(operators)>spec["max_operators"]:
                    raise XCValidationError(f"Checkpoint morphogenesis {name} exceeds declared capacity")
                ids=set()
                checked_nodes=[]
                for node in nodes:
                    vec=node.get("vector")
                    if shape(vec)!=(len(self.field_names),):
                        raise XCValidationError(f"Checkpoint morphogenesis {name} has invalid Ψ vector")
                    nid=str(node.get("id"))
                    if nid in ids: raise XCValidationError(f"Checkpoint morphogenesis {name} has duplicate Ψ id {nid}")
                    ids.add(nid)
                    checked_nodes.append({
                        "id":nid,
                        "kind":str(node.get("kind","experience")),
                        "vector":[float(x) for x in vec],
                        "created_event":int(node.get("created_event",-1)),
                        "salience":float(node.get("salience",0.5)),
                    })
                if not checked_nodes:
                    checked_nodes=[{
                        "id":"psi_core","kind":"core","vector":list(self.state),
                        "created_event":-1,"salience":1.0,
                    }]
                    ids={"psi_core"}
                # Exact checkpoint replay must preserve the stored structural core vector.
                # The live morph() operation updates psi_core when a structural event
                # triggers; loading a checkpoint must not silently rewrite history.

                checked_connections=[]
                for edge in connections:
                    a=str(edge.get("source")); b=str(edge.get("target"))
                    if a not in ids or b not in ids:
                        raise XCValidationError(f"Checkpoint morphogenesis {name} connection references unknown Ψ node")
                    checked_connections.append({
                        "source":a,"target":b,
                        "weight":float(edge.get("weight",0.5)),
                        "type":str(edge.get("type","experience_growth")),
                        "created_event":int(edge.get("created_event",-1)),
                    })

                checked_ops=[]
                for op in operators:
                    target=str(op.get("target_matrix"))
                    if target not in spec["target_matrices"]:
                        raise XCValidationError(f"Checkpoint morphogenesis {name} operator targets undeclared matrix {target}")
                    mat=op.get("matrix")
                    if shape(mat)!=(len(self.field_names),len(self.field_names)):
                        raise XCValidationError(f"Checkpoint morphogenesis {name} operator matrix shape mismatch")
                    checked_ops.append({
                        "id":str(op.get("id")),
                        "target_matrix":target,
                        "matrix":[[float(x) for x in row] for row in mat],
                        "gain":float(op.get("gain",0.0)),
                        "created_event":int(op.get("created_event",-1)),
                    })

                self.morph_state[name]={
                    "attempts":int(stored.get("attempts",0)),
                    "events":int(stored.get("events",0)),
                    "last_triggered":bool(stored.get("last_triggered",False)),
                    "last_kind":str(stored.get("last_kind","NONE")),
                    "nodes":checked_nodes,
                    "connections":checked_connections,
                    "operators":checked_ops,
                }
...
def print_event_run(result):
    ...
    for s in result["trace"]:
        ...
        morph=" ".join(
            f"{n}:Psi{d['nodes']}/tau{d['connections']}/op{d['operators']}/e{d['events']}"
            for n,d in s.get("morphology",{}).items()
        )
        print(f"event={s['event_index']:>2} {s['event']:<12} obs={str(s['observation']):<12} action={str(s['action']):<12} {st} memory=[{mem}] morph=[{morph}]")
    print("-")
...
def cmd_check(args):
    ...
    print(f"morphogenesis:{', '.join(p.get('morphogenesis',{})) or '(none)'}")
...
def cmd_checkpoint(args):
    ...
    for n,s in data.get('morphogenesis',{}).items():
        print(
            f"morph {n}:      Ψ={len(s.get('nodes',[]))} "
            f"τ={len(s.get('connections',[]))} ops={len(s.get('operators',[]))} "
            f"events={s.get('events',0)} attempts={s.get('attempts',0)}"
        )
...
def build_cli():
    p=argparse.ArgumentParser(prog="xc",description="XC compiler/interpreter v0.3.1.2")
